"""Configuration loading for quality and entropy formatters.

Provides YAML-based threshold configuration with resolution hierarchy:
    defaults (code) -> domain (YAML) -> dataset (YAML) -> runtime (API)

This module was moved from quality/formatting/config.py to core/formatting/
to support shared use by both quality and entropy layers.

Usage:
    from dataraum_context.core.formatting.config import load_formatter_config
