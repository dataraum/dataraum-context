...
- **Composite key detection** — multi-column join candidates. Currently only single-column joins are detected. Multi-tenant datasets often require composite keys (e.g., `Business Id` + `Customer name`) for grain-safe joins. Evidence from example data:
  - No single column in any dimension table is unique (all single-column joins are many-to-many)
  - `payment_method` with composite key `(Business Id, Payment method)` is grain-safe with 100% RI
  - `product_service` with composite key `(Business Id, Product_Service)` is grain-safe with 26% RI
  - `chart_of_account` composite `(Business Id, Account name)` still has duplicates (2213/2430 unique)
  - Implementation approach: detect shared columns (e.g., `Business Id` appears in both tables), try composite keys, verify uniqueness. Store as multi-column relationship with `from_columns`/`to_columns` arrays. Enriched views phase would need to generate multi-column JOIN ON clauses
